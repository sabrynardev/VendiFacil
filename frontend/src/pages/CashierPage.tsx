...
export function CashierPage() {
  ...
  async function refreshOperation() {
    const [current, waiting, customerList] = await Promise.all([cashRegisterService.current(), salesService.list({ status: "ON_HOLD" }), customerService.list()]);
  ...
  function clearSale() { setCart([]); setDiscount(0); setSurcharge(0); setNote(""); setHeldId(null); setPayments([{ method: "PIX", amount: 0 }]); setAuthorizationEmail(""); setAuthorizationPassword(""); setCustomerId(0); setCreditDueDate(""); }

  async function finalize() {
    setProcessing(true); setError("");
    try {
      const payload = { items: cart.map((i) => ({ product_id: i.product.id, quantity: i.quantity, discount: i.discount })), discount, surcharge, payments, note, idempotency_key: crypto.randomUUID(), authorization_email: authorizationEmail || undefined, authorization_password: authorizationPassword || undefined, customer_id: customerId || undefined, credit_due_date: creditDueDate || undefined };
