...
def list_sales(
    ...
    current_user: User = Depends(require_permission(PermissionCode.SALES_VIEW)),
):
...
def create_sale_endpoint(
    payload: SaleCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission(PermissionCode.CASHIER_OPERATE)),
):
    try:
    ...
    log_audit(
        ...
        entity_type="SALE",
        entity_id=sale.id,
        description=f"Concluiu a venda #{sale.id} no valor de R$ {float(sale.total):.2f}.",
    )
    db.commit()
    return serialize_sale(sale)
